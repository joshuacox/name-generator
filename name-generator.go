...
func main() {
	...
	// Separator (default "-") – mirrors the shell script's SEPARATOR variable.
	...
	for i := 0; i < counto; i++ {
		...
		// Use the separator variable instead of a hard‑coded hyphen.
		name := fmt.Sprintf("%s%s%s", thisAdj, separator, thisNoun)
