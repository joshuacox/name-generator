...
fn pickRandomFile(allocator: std.mem.Allocator, dir_path: []const u8) ![]const u8 {
    ...
    var files = std.ArrayList([]const u8).initCapacity(allocator, 1);
    defer files.deinit();
    ...
    while (try it.next()) |entry| {
        if (entry.kind == .file) {
            ...
            try files.appendAssumedCapacity(allocator, full_path);
        }
    }
...
pub fn main() !void {
    ...
    // getCwdAlloc expects the allocator value.
    const cwd = try std.process.getCwdAlloc(allocator);
    defer allocator.free(cwd);

    // Join paths using the allocator value (no pointer needed for the newer API).
